...
```text
Contribution -> Evidence -> Memory Object -> Evidence Quality -> Attestation -> Lifecycle State -> Protocol Memory Layer
```

A Memory Object is the structured record that Chronicle remembers. It is not the contribution itself.
...
A Memory Object must be able to answer five basic questions:

1. What is being remembered?
2. Who or what is associated with it?
3. What evidence supports it?
4. What state is it in?
5. How does it relate to other memory records?
...
| `evidence_quality` | enum or object | Object has entered `observed`, `under_review`, `accepted`, or `verified` states | Describes evidence confidence and limitations |
...
| `archive_refs` | array | Related Chronicle Archive entries or external archive references |
...
| `derived_from` | Object was created using prior source material |
| `cited_by` | Object is cited by another object |
...
Relationship fields should not be ambiguous. A link should specify whether it references, inherits from, extends, disputes, corrects, supersedes, or duplicates another object.
...
| `submitted` | Required core fields should be present |
| `observed` | Evidence source must be inspectable or explicitly marked unavailable |
| `under_review` | Review notes or reviewer references should be present |
...
|---|---|
| Fake contribution | Requires evidence references and lifecycle state |
...
| Privacy leakage | Includes privacy and redaction fields |
...
| AI overconfidence | Supports lifecycle, evidence quality, and retrieval notes |
...
It clarifies what every Memory Object should contain, which fields are state-dependent, how relationships should be represented, and which fields should remain outside the base schema to avoid premature scoring or reward logic.
